...
{
  Predictor pred(exceed_penalty_, aa_int_max_, rna_int_max_, mu_, nu_, n_th_);
  VVF int_weight;
  VF aa_weight, rna_weight;
  fm.calculate_feature_weight(aa, rna, int_weight, aa_weight, rna_weight);
  float score_for_correct = pred.calculate_score(int_weight, aa_weight, rna_weight, correct_int);

  if (max_margin)
    penalize_correct_interaction(int_weight, aa_weight, rna_weight, correct_int);
  VVU predicted_int;
  pred.predict_interaction(fm, aa, rna, int_weight, aa_weight, rna_weight, predicted_int, mu);
  float score_for_predict = pred.calculate_score(int_weight, aa_weight, rna_weight, predicted_int);
...
cross_validation(uint n)
{
  ...
  for (uint i=0; i!=n; ++i) {
    ...
    for (auto j : test) {
      VVU predicted_int;
      pred.predict_interaction(fm, labeled_aa_[j], labeled_rna_[j], predicted_int);
    ...
    for (uint u=0; u!=g_max_; ++u)
    {
      ...
      for (auto i : idx)
      {
        ...
        else                    // unlabeled data
        {
          ...
          VVVU common_int;
          pred.predict_common_interaction(fm, unlabeled_aa_[iu], unlabeled_rna_[iu], common_int);
...
run()
{
  ...
  else
  {
    ...
    VVU predicted_int;
    float s=pred.predict_interaction(fm, aa, rna, predicted_int);
