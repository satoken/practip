...
void
ssvm::
extract_feature(uint i, uint j, Func& func) const
{
  char buf[20];
...
struct EdgeWeight
{
  EdgeWeight(const std::map<std::string,float>& feature_weight,
             VVF& edge_weight)
  ...
  const std::map<std::string,float>& feature_weight_;
  VVF& edge_weight_;
};
